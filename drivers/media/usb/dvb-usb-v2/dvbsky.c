...
#if 0 //IS_ENABLED(CONFIG_RC_CORE)
...
#ifdef MYGICA
static const struct m88ds3103_config mygica_hdstar_m88ds3103_config = {
	.i2c_addr = 0x68,
	.clock = 27000000,
	.i2c_wr_max = 33,
	//.clock_out = 0,
	...
	.agc = 0x99,
	.agc_inv = 0,
	.clock_out = M88DS3103_CLOCK_OUT_DISABLED,
	.envelope_mode = 0,
	.lnb_hv_pol = 0,
	.lnb_en_pol = 1,
};
...
#ifdef MYGICA
static int mygica_hdstar_attach(struct dvb_usb_adapter *adap)
{
	struct dvbsky_state *state = adap_to_priv(adap);
	struct dvb_usb_device *d = adap_to_d(adap);
	int ret = 0;
	...
	/* attach demod */
	adap->fe[0] = dvb_attach(m88ds3103_attach,
			&mygica_hdstar_m88ds3103_config,
			&d->i2c_adap,
			&i2c_adapter);
	if (!adap->fe[0]) {
		dev_err(&d->udev->dev, "mygica_hdstar_attach fail.\n");
		ret = -ENODEV;
		goto fail_attach;
	}
...
#ifdef MYGICA
static int mygica_t230_attach(struct dvb_usb_adapter *adap)
{
	struct dvbsky_state *state = adap_to_priv(adap);
	struct dvb_usb_device *d = adap_to_d(adap);
	int ret = 0;
	...
	if (client_demod == NULL ||
			client_demod->dev.driver == NULL) {
		dev_err(&d->udev->dev, "%s: fail_demod_device.\n", __func__);
		goto fail_demod_device;
	}
	if (!try_module_get(client_demod->dev.driver->owner)) {
		dev_err(&d->udev->dev, "%s: fail_demod_module.\n", __func__);
		goto fail_demod_module;
	}
	...
	if (client_tuner == NULL ||
			client_tuner->dev.driver == NULL) {
		dev_err(&d->udev->dev, "%s: fail_tuner_device.\n", __func__);
		goto fail_tuner_device;
	}
	if (!try_module_get(client_tuner->dev.driver->owner)) {
		dev_err(&d->udev->dev, "%s: fail_tuner_module.\n", __func__);
		goto fail_tuner_module;
	}
...
static struct dvb_usb_device_properties mygica_t230_props = {
	...
	.i2c_algo         = &dvbsky_i2c_algo,
	.frontend_attach  = mygica_t230_attach,
	.init             = dvbsky_init,
	.get_rc_config    = dvbsky_get_rc_config,
	.streaming_ctrl   = dvbsky_streaming_ctrl,
	.identify_state	  = dvbsky_identify_state,
...
static struct dvb_usb_device_properties mygica_hdstar_props = {
	...
	.i2c_algo         = &dvbsky_i2c_algo,
	.frontend_attach  = mygica_hdstar_attach,
	.init             = dvbsky_init,
	.get_rc_config    = dvbsky_get_rc_config,
	.streaming_ctrl   = dvbsky_streaming_ctrl,
	.identify_state	  = dvbsky_identify_state,
